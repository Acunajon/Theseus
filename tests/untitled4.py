...
Parameters = [wave, T, Norm, T_Hot, Norm_Hot, T_Cold, Norm_Cold]
...
chi = lambda Theo, Dat, Sig: sum(((Dat-Theo)**2)/Sig**2)

print chi(Theory, Obs, uncert)

asdfq = chi(Theory, Obs, uncert)



def Chi_2(Dat, Theo, Uncert):
    Chi = 0
    for i in range(len(Dat)):
        Chi += ((Dat[i] - Theo[i])**2)/(Uncert[i]**2)
    return Chi
    
print Chi_2(Obs, Theory, uncert)

print 'Upper is Chi_2' 
...
res = scipy.optimize.minimize(Planck_3, Parameters, method='nelder-mead', 
              tol=1e-6)
